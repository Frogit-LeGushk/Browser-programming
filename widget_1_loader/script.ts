...
interface IDrawProps extends IState {
    xc: number
    yc: number
    r1: number
    r2: number
...
function defaultSettingsProps(theme: boolean): ISettings {
    return {
        theme,
        d1: 0,
        d2: 120,
...
function setTheme(props: IState) {
    let prevTheme = true;
   
    return () => {
        if(prevTheme != props.theme)
        {
            if(props.theme) document.body.className = "theme_light";
            else            document.body.className = "theme_dark";
            prevTheme = props.theme;
        } 
    }
}
function draw(props: IDrawProps): void {
    const rad1: number  = (2 * Math.PI * props.d1) / 360;
    const rad2: number  = (2 * Math.PI * props.d2) / 360;
    const rad3: number  = (2 * Math.PI * props.d3) / 360;
...
function drawWrapper(props: IState, wrapper: HTMLElement): void {
    const scaling = 0.9;
    ...
    if(wrapper.offsetWidth < wrapper.offsetHeight)
    {
        ...
        if(wrapper.offsetWidth >= 500) k = 1;
        if(wrapper.offsetWidth <= 300) k = endValueSVGScaling;
    }
    else
    {
        r3 = scaling * (wrapper.offsetHeight / 2);
        k = m * wrapper.offsetHeight;
        if(wrapper.offsetHeight >= 500) k = 1;
        if(wrapper.offsetHeight <= 300) k = endValueSVGScaling;
    }
    ...
    draw({
        xc: Math.round(wrapper.offsetWidth / 2),
        yc: Math.round(wrapper.offsetHeight / 2),
        r1: Math.round(1 * r3 / 3),
        r2: Math.round(2 * r3 / 3),
        r3: Math.round(r3),
        k,
        ...props,
    })
...
window.addEventListener("DOMContentLoaded", () => {
    const __allElementsSvg: NodeListOf<SVGElement> = document.querySelectorAll("main > svg");
    const __allElementsSpan: NodeListOf<HTMLSpanElement> = document.querySelectorAll("main > span");
    // @ts-ignore
    const __main: HTMLElement = document.querySelector("main");

    let state: IState = {
        ...defaultSettingsProps(true),
        f: __allElementsSvg[0],
        b1: __allElementsSvg[1],
        b2: __allElementsSvg[2],
        b3: __allElementsSvg[3],
        c1: __allElementsSpan[0],
        c2: __allElementsSpan[1],
        c3: __allElementsSpan[2]
    }
    ...
    window.onmessage = (event: MessageEvent) => {
        try {
            ...
            if(parsedData && parsedData.hasOwnProperty("theme"))
            {
                ...
                event.source?.postMessage(JSON.stringify({res: true}), {targetOrigin: event.origin || "*"});
            }
        }
